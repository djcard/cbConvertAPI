		...
		describe(
			...
			body   = function(){
				beforeEach( function(){
					...
					testObj.setdestTypesSupported( fakeTypes );
					testme = testObj.typeToSupported( fakeTypes[ randRange( 1, fakeTypes.len() ) ] );
				} );
				...
				it( "If the type submitted is NOT in the sourceTypeSupported, return true ", function(){
					testme = testObj.typeToSupported( "FGIGDIGIDFIFDHIFdfdfgfd" );
