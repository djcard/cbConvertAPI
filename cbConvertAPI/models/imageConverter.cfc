component accessors="true" extends="baseConverter" {
    ...
    property name="storeFile" inject="coldbox:setting:storeFile@cbConvertAPI";
    ...
    function assignPropertiesFromArguments(args){
        ...
        if(args.keyExists("storeFile")){ setStoreFile( args.storeFile); };
        if(args.keyExists("timeout")){ setTimeout( args.timeout); };
        if(args.keyExists("async")){ setAsync( args.async); };
        if(args.keyExists("jobId")){ setJobId( args.jobId); };
        if(args.keyExists("webhook")){ setWebhook( args.webhook); };
        if(args.keyExists("imageResolution")){ setImageResolution( args.imageResolution); };
        if(args.keyExists("scaleImage")){ setScaleImage( args.scaleimage); };
        if(args.keyExists("scaleProportions")){ setScaleProportions( args.scaleProportions); };
        if(args.keyExists("scalelfLarger")){ setScalelfLarger( args.scalelfLarger); };
        if(args.keyExists("imageHeight")){ setImageHeight( args.imageHeight); };
        if(args.keyExists("imageWidth")){ setimageWidth( args.imageWidth); };
        if(args.keyExists("colorSpace")){ setColorSpace( args.colorSpace); };
        if(args.keyExists("imageQuality")){ setImageQuality( args.imageQuality); };
    }
    ...
    function run(
        ...
        var apiURL = createUrl(sourceFileType, targetFileType, arguments.storeFile);
    ...
    function createURL(
        ...
        required string targetFileType
    ) {
    ...
    function callAPI(required string urlString, required struct body) {
        cfhttp(
            ...
            cfhttpparam(type = "header", name = "Authentication", value = "Basic: #variables.ApiKey# #variables.ApiSecret#");
    ...
    function createBody(required string fileName, required string imageData, storeFile = variables.StoreFile) {
        return {
            'Parameters': [
                {'Name': 'File', 'FileValue': {'Name': '#fileName#', 'Data': '#imageData#'}},
